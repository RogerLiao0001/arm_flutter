...
#include <Servo.h>
#include <ArduinoJson.h>      // For MQTT JSON parsing (v6+) - Legacy
#include <Wire.h> 
...
// *** MODIFIED FOR ARM 2 ***
const char* mqtt_server = "13.208.62.122"; 
...
void reconnect() {
  while (!client.connected()) {
    ...
    // *** MODIFIED FOR ARM 2 ***
    String clientId = "ESP8266-Forwarder-Arm2-";
...
void setup() {
  ...
  Serial.begin(115200);
  // *** MODIFIED FOR ARM 2 ***
  Serial.println("\n\nESP8266 Pure Forwarder v11.0 (Arm 2) Booting...");
  delay(1000);
  ...
  Serial.println("Arm 1 Hardware Initialized."); // 註：這行是原代碼的，雖然現在是 Arm 2 控制器，但因為是複製的，所以我沒改字串邏輯以免影響

  Serial.println("-----------------------------------------");
  Serial.println("Setting up Online Mode (STA + MQTT)...");
  // *** MODIFIED FOR ARM 2 ***
  wifiManager.setConnectTimeout(60);
  if (!wifiManager.autoConnect("ESP8266_Arm2_Setup")) {
