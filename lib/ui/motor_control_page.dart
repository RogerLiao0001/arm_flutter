...
class _LiveKitReceiverWidgetState extends State<LiveKitReceiverWidget> {
  ...
  bool _isYoloOverlayEnabled = true; // 預設開啟
  ...
  Widget build(BuildContext context) {
    ...
    if (_connecting) { content = const Center(child: CircularProgressIndicator(color: Colors.white)); }
      ...
      // *** 使用 Stack 疊加 YOLO overlay ***
      content = Stack(
        children: [
          // 底層：影像播放器
          lk.VideoTrackRenderer(_videoTrack!),

          // 中層：YOLO overlay（只在啟用且有偵測結果時顯示）
          if (_isYoloOverlayEnabled && _yoloDetections.isNotEmpty)
            Positioned.fill(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  return CustomPaint(
                    painter: YoloOverlayPainter(
                      ...
                      videoSize: Size(constraints.maxWidth, constraints.maxHeight),
                    ),
                  );
                },
              ),
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: _isYoloOverlayEnabled ? Colors.green : Colors.grey,
                  width: 2,
                ),
              ),
              child: IconButton(
                ...
                iconSize: 18,
                icon: Icon(
                  _isYoloOverlayEnabled ? Icons.visibility : Icons.visibility_off,
                  color: _isYoloOverlayEnabled ? Colors.green : Colors.grey,
                ),
                onPressed: () {
                  setState(() {
                    _isYoloOverlayEnabled = !_isYoloOverlayEnabled;
                  });
                },
                tooltip: _isYoloOverlayEnabled ? 'Hide YOLO Overlay' : 'Show YOLO Overlay',
              ),
            ),
          ),
        ],
...
class _MotorControlPageState extends ConsumerState<MotorControlPage> {
  ...
  void _publishIKState(MqttService mqttService) {
    ...
    while (precision >= 0) {
      ...
      if (payload.length <= 32) {
        print("Publishing IK (P:$precision): $payload (${payload.length} bytes)");
    ...
    payload = "IK $x $y $z ${rxRad.round()} ${ryRad.round()} ${rzRad.round()}";
    print("Publishing IK (P:0, as fallback): $payload (${payload.length} bytes)");
    _publishThrottled(mqttService, 'ik_gamepad', IKConfig.ikTopic, payload);
  }
