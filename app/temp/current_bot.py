...
FRAME_INTERVAL = 10  # 每 10 幀處理一次（降低記憶體）
...
class AIBot:
    ...
    async def broadcast_model_list(self):
        ...
        logging.info(f"Broadcasted model list")
    ...
    async def process_video_frames(self, track: rtc.VideoTrack):
        ...
        async for event in video_stream:
            ...
            if self.frame_count % 30 == 0:
                logging.info(f"Received {self.frame_count} frames total")
            ...
            try:
                ...
                arr = np.frombuffer(buffer.data, dtype=np.uint8).reshape((buffer.height, buffer.width, 4))
                arr = arr[:, :, [2, 1, 0]]

                # YOLO 推論（降低confidence threshold來增加偵測率）
                results = self.current_model.predict(
                    arr,
                    verbose=False,
                    device=self.device,
                    conf=0.15  # 降低到0.15以增加偵測機會
                )

                detections = []
                for r in results:
                    for box in r.boxes:
                        # 使用 xywhn（新版YOLO API）- normalized [x_center, y_center, width, height]
                        xywhn_data = box.xywhn[0].cpu().numpy() if hasattr(box.xywhn[0], 'cpu') else box.xywhn[0]
                        x_center, y_center, width, height = xywhn_data.tolist()
                        x = x_center - (width / 2)
                        y = y_center - (height / 2)
                ...
                if detections:
                    logging.info(f"Frame {self.frame_count}: ✅ Detected {len(detections)} objects - SENDING TO CLIENTS")
                    await self._send_detections(detections)
                elif self.frame_count % 100 == 0:
                    logging.info(f"Frame {self.frame_count}: No objects detected")

            except Exception as e:
                logging.error(f"YOLO prediction error: {e}", exc_info=True)

    async def _send_detections(self, detections):
        if not self.room:
            logging.warning("⚠️ Cannot send detections: No room connection")
            return
        payload = json.dumps(detections).encode('utf-8')
        try:
            await self.room.local_participant.publish_data(payload, reliable=True)
            logging.info(f"✅ Successfully sent {len(detections)} detection(s) to viewers")
        except Exception as e:
            logging.error(f"❌ Failed to send detections: {e}")
