...
class _MotorControlPageState extends ConsumerState<MotorControlPage> {
  ...
  static const double _initialMotorA = 90.0, _initialMotorC = 90.0, _initialMotorE = 90.0, _initialMotorH = 100.0;
  double _motorA_angle = _initialMotorA;
  double _motorC_angle = _initialMotorC;
  double _motorE_angle = _initialMotorE;
  double _motorH_angle = _initialMotorH;
  ...
  void _resetGamepadToDefaults(MqttService mqttService) {
    ...
    print("Gamepad controls have been reset to default values.");
  }
    ...
    return GestureDetector(
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Icon(icon, size: 36.0, color: Theme.of(context).primaryColor),
        ),
      ),
    );
  }
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        _buildContinuousControlButton(
                          ...
                          onUpdate: () {
                            setState(() => _motorC_angle = (_motorC_angle + 1.0).clamp(30.0, 90.0));
                        ...
                        Row(
                          children: [
                            _buildContinuousControlButton(
                              ...
                              onUpdate: () {
                                setState(() => _motorA_angle = (_motorA_angle - 1.0).clamp(45.0, 135.0));
                    ...
                    Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        const Text("Claw", style: TextStyle(fontWeight: FontWeight.bold)),
                        const SizedBox(height: 10),
                        ElevatedButton.icon(
                          icon: const Icon(Icons.lock_open), label: const Text("Open"),
                          style: ElevatedButton.styleFrom(minimumSize: const Size(120, 50)),
                          onPressed: () {
                            setState(() => _motorH_angle = 100);
                            mqttService.publish('servo/arm2/h', jsonEncode({"angle": 100}));
                          },
                        ),
                        const SizedBox(height: 20),
                        ElevatedButton.icon(
                          icon: const Icon(Icons.lock_outline), label: const Text("Close"),
                          style: ElevatedButton.styleFrom(minimumSize: const Size(120, 50)),
                          onPressed: () {
                     ...
                     Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        const Text("Forward", style: TextStyle(fontWeight: FontWeight.bold)),
                        const SizedBox(height: 10),
                        _buildContinuousControlButton(
                          icon: Icons.arrow_upward,
                          onUpdate: () {
                            setState(() => _motorE_angle = (_motorE_angle + 1.0).clamp(45.0, 135.0));
  ...
  // ** FIX: This widget is now only used for the Slider page layout **
  Widget _buildArm2Controls(MqttService mqttService) {
     ...
     return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            // ** FIX: Integrate Claw controls into the title row to save vertical space **
            Row(
              children: [
            ...
            // ** FIX: Revert slider area to its original structure inside Expanded **
            Expanded(
              child: SingleChildScrollView(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
         ...
         // --- 新增的按鈕 ---
         IconButton(
           icon: const Icon(Icons.flip_camera_android_outlined), // 攝影機圖示
           ...
           onPressed: () async {
             ...
             if (await canLaunchUrl(publisherUrl)) {
               // 啟動 URL，mode: LaunchMode.externalApplication 會在新的瀏覽器分頁中打開
      ...
      body: Column( children: [
          ...
          Expanded( flex: 6, child: Container( color: Colors.grey[200], padding: const EdgeInsets.fromLTRB(8.0, 0, 8.0, 8.0), child: Column( crossAxisAlignment: CrossAxisAlignment.stretch, children: [
             Padding( padding: const EdgeInsets.symmetric(vertical: 8.0), child: Center( child: ToggleButtons(
               ...
               constraints: const BoxConstraints(minHeight: 38.0, minWidth: 140.0),
               children: const <Widget>[ Text('Gamepad Control'), Text('Slider Control'), ],
              ),),),
