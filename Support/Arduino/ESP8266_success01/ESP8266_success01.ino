...
// --- 腳位定義 ---
// --- Arm 1 (直接控制) ---
#define SERVO1_PIN 2    // GPIO5 (NodeMCU D4) - Arm 1 Servo 1
#define SERVO2_PIN 16    // GPIO4 (NodeMCU D0) - Arm 1 Servo 2
...
// --- Arm 2 (Serial 數據線輸出) ---：GPI04 (D2)為SCL(Clock)，GPI05 (D1)為 SDA (Data) gnd-gnd
...
// Arm 2: servo/arm2/a, servo/arm2/b, ..., servo/arm2/f
...
const long ARM2_SERIAL_BAUDRATE = 57600; // *** 已更新為您範例中的 57600 ***
...
// ======================================================
//  Web Server HTML 頁面產生函式 (Offline Mode) - *** 已修正 ***
// ======================================================
String buildHtmlPage(String page) {
  ...
  if (page == "arm1") {
      ...
      // Arm 2 UI uses IDs 1-6, which map to labels a-f
      char arm2_labels[] = {'a', 'b', 'c', 'd', 'e', 'f'};
      for (int i = 1; i <= 6; i++) { // Loop for 6 motors
          html += "<div class='control-group'>";
          String motorTypeLabel = (i <= 3) ? "伺服馬達" : "步進馬達"; // Example, actual type is handled by receiving ESP32
          
          // --- *** 修正開始 *** ---
          String tempLabelChar = String(arm2_labels[i-1]); // 建立一個 String 物件 e.g., "a"
          tempLabelChar.toUpperCase();                      // 將 tempLabelChar 修改為 "A"
...
void handleSetArm2Motor() {
  if (server.hasArg("id") && server.hasArg("angle")) {
      ...
      if (motorLabel != ' ') {
          ...
          Wire.beginTransmission(SLAVE_ADDRESS); //呼叫二號開始傳送
          Wire.write(serialCommand.c_str()); //傳送字串,舉例:[a150],請在結尾加入'|'符號,才能讀取到,傳送出去的是[a130|]
          Wire.endTransmission(); //停止傳送關閉連接
          Serial.printf("[Web] Arm 2 Motor: WebID=%d -> Label=%c, Angle=%d -> Serial: %s\n", webMotorId, motorLabel, angle, serialCommand.c_str());
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (subTopic.equals("magnet")) {
      ...
      if (subTopic.length() == strlen("arm2/") + 1) {
          ...
          if (arm2Label >= 'a' && arm2Label <= 'f') {
              DynamicJsonDocument doc(JSON_DOC_SIZE);
              DeserializationError error = deserializeJson(doc, message);
              if (error || !doc.containsKey("angle")) {
                  Serial.printf("Arm 2 (Label %c): JSON parsing failed or missing 'angle' [%s]\n", arm2Label, error.c_str()); return;
              }
              int angle = doc["angle"]; angle = constrain(angle, 0, 180);
              String serialCommand = "[" + String(arm2Label) + String(angle) + "]|";
              Serial.printf("Processing Arm 2: Label=%c, Angle=%d -> Serial: %s\n", arm2Label, angle, serialCommand.c_str());
              Serial1.print(serialCommand);
          } else { Serial.printf("Invalid Arm 2 label: %c\n", arm2Label); }
      } else { Serial.println("Invalid Arm 2 sub-topic format."); }
  } else if (subTopic.toInt() >= 1 && subTopic.toInt() <= 5) {
      DynamicJsonDocument doc(JSON_DOC_SIZE);
      DeserializationError error = deserializeJson(doc, message);
      ...
      if (!error && doc.containsKey("id") && doc.containsKey("angle")) {
          ...
          if (subTopic.toInt() != servoId) { Serial.printf("Warning: Topic ID (%s) mismatch with payload ID (%d)\n", subTopic.c_str(), servoId); }
      ...
      } else { Serial.printf("Arm 1 Servo (Topic %s): JSON parsing failed or invalid format [%s]\n", subTopic.c_str(), error.c_str()); }
  } else { Serial.println("Unrecognized sub-topic under servo/"); }
}

// ======================================================
//  MQTT Reconnect Logic (Online Mode) - *** 已更新 Subscription ***
// ======================================================
void reconnect() {
...
void setupOfflineMode() {
  ...
  if (WiFi.softAP(ap_ssid)) {
      Serial.println("\nAP Mode Active! Network is OPEN."); Serial.print("SSID: "); Serial.println(ap_ssid);
      IPAddress myIP = WiFi.softAPIP(); Serial.print("AP IP address: "); Serial.println(myIP);
      if (dnsServer.start(53, "*", myIP)) { Serial.println("DNS Server started."); }
      else { Serial.println("Failed to start DNS Server!"); }
      ...
      server.onNotFound([]() { Serial.println("Handling not found (Captive Portal?) -> Redirecting to root."); handleRoot(); });
      server.begin(); Serial.println("Web Server started.");
      Serial.println("Connect to OPEN WiFi and control page should appear.");
  } else { Serial.println("Failed to start AP!"); }
}
...
void setup() {
  Wire.begin();
  Serial.begin(115200);
  Serial.println("\n\nESP8266 Dual Robot Arm Controller v2.4 Booting..."); // Version bump
  delay(1000);

  Serial1.begin(ARM2_SERIAL_BAUDRATE); // *** ARM2_SERIAL_BAUDRATE 在上面已修改 ***
